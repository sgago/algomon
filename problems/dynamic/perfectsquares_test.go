...
func TestPerfectSquaresBfs(t *testing.T) {
	actual := perfectSquaresBfs(15)
	fmt.Println("count:", actual)
...
func perfectSquaresBfs(target int) int {
	if target == 0 {
		return 1
	}
	...
	for !q.Empty() {
		...
		for _, sqr := range sqrs {
			...
			if next == target {
				...
				fmt.Println("equation", curr.str+"+"+fmt.Sprint(sqr))
				return count
			}
			...
			// perfect sqrs. We got to next in some more efficent
...
// FIXME: Bottom up is not fixed yet. This is brute, pretty sure.
func TestPerfectSquaresDp(t *testing.T) {
	actual := perfectSquaresDp(15)
	fmt.Println("count:", actual)
	assert.Equal(t, 4, actual)
}

func perfectSquaresDp(target int) int {
	if target == 0 {
		return 1
	}
	...
	for i := 1; i <= target; i++ {
		for _, sqr := range sqrs {
			...
			if val, ok := memo[next]; ok {
				// We've cached next into memo, so take the min of it
				memo[next] = min(memo[i]+1, val)
			} else {
				// New value not in memo, will be memo[i]+1
				memo[next] = memo[i] + 1
			}
		}
	}
